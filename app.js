(() => {
  ...
  const el = {
    ...
    songMeta: $('songMeta'),
    time: $('timeDisplay'),
    copyTime: $('copyTimeButton'),
    start: $('startMarkButton'),
    end: $('endMarkButton'),
    length: $('rangeLength'),
    back: $('backButton'),
    play: $('playButton'),
    ...
    timelineStatic: $('timelineStaticCanvas'),
    timelineCursor: $('timelineCursorCanvas'),
    overviewViewport: $('overviewViewport'),
    overviewStatic: $('overviewStaticCanvas'),
    overviewCursor: $('overviewCursorCanvas'),
    purpose: $('purposeSelect'),
    fade: $('fadeSelect'),
    preview: $('outputPreview'),
    copyOut: $('copyOutputButton'),
    debug: $('audioDebug'),
    errorCard: $('errorCard'),
    error: $('errorMessage'),
  };
  ...
  function buildEffectBuffer(activeMap, music, buffers, audioContext) {
    ...
    for (const hit of activeMap.hits) {
      ...
      if (nominalStart >= effect.length) continue;
      ...
      for (let ch = 0; ch < channels; ch++) {
        ...
        const srcCh = Math.min(ch, hs.numberOfChannels - 1);
        const srcData = hs.getChannelData(srcCh);
  ...
  async function buildMapEffect() {
    ...
    await loadHitsounds();
    effectBuffer = buildEffectBuffer(map, musicBuffer, hsBuffers, ac);
  }
  ...
  function drawDynamic(positionSec = audiblePosition()) {
    ...
    updateTimelineTransform(ms);
    drawTimelineCursor();
    drawOverviewCursor(positionSec);
  }
  ...
  function timelineTargetFromEvent(event) {
    ...
    return clampSec((timelineScrub?.basePosition ?? pausedOffset) + ((q - 0.5) * spanMs()) / 1000);
  }
